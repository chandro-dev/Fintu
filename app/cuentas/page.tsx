...
import type { Cuenta, Transaccion, TxForm } from "@/components/transactions/types";
...
export default function CuentasPage() {
  ...
  const { session, loadingSession, cuentas, transacciones, categorias, refresh } = useAppData();
  ...
  const tipoNormalId = useMemo(
    () =>
      tipos.find(
        (t) =>
          t.codigo?.toUpperCase?.() === "NORMAL" || t.codigo?.toUpperCase?.() === "CUENTA_NORMAL",
      )?.id,
    [tipos],
  );
      ...
      if (editingId && saldoObjetivo !== "") {
        ...
        if (!Number.isNaN(deseado)) {
          ...
          if (delta !== 0) {
            ...
            if (ajusteNota.trim()) payload.ajusteDescripcion = ajusteNota.trim();
          }
        }
      }
  ...
  const startEdit = (cta: ExtendedCuenta) => {
    ...
    setSaldoObjetivo(
      cta.saldo !== undefined && cta.saldo !== null ? String(Number(cta.saldo ?? 0)) : "",
    );
    setAjusteNota("");
    setShowModal(true);
  };
  ...
  const validateTx = (data: TxForm) => {
    ...
    if (!data.monto || Number(data.monto) <= 0) return "Monto debe ser mayor a 0";
    return null;
  };
  ...
  return (
    <div className="min-h-screen px-6 py-10 text-slate-900 dark:text-zinc-50">
      <div className="mx-auto flex max-w-5xl flex-col gap-6">
        ...
        {loadingSession && <p className="text-sm text-slate-500">Cargando sesion...</p>}
        ...
        {cuentaSeleccionada && (
          <section className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-lg dark:border-white/10 dark:bg-black/30">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-4 dark:border-white/10">
              <div>
                <p className="text-xs uppercase tracking-[0.3em] text-sky-500">Detalle</p>
                <h2 className="text-2xl font-semibold text-slate-900 dark:text-white">
                  {cuentaSeleccionada.nombre}
                </h2>
                <p className="text-sm text-slate-600 dark:text-zinc-400">
                  Saldo actual:{" "}
                  {formatMoney(Number(cuentaSeleccionada.saldo ?? 0), cuentaSeleccionada.moneda)}
                </p>
              </div>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-3">
              <StatCard
                label="Ingresos"
                value={formatMoney(txSummary.ingresos, cuentaSeleccionada.moneda)}
                tone="emerald"
              />
              <StatCard
                label="Egresos"
                value={formatMoney(txSummary.egresos, cuentaSeleccionada.moneda)}
                tone="rose"
              />
              <StatCard
                label="Neto"
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              <div className="rounded-xl border border-slate-200/70 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-black/40">
                <p className="text-sm font-semibold text-slate-900 dark:text-white">Flujo 6 meses</p>
                <div className="mt-3 space-y-2">
                  {flowByMonth.length === 0 && (
                    <p className="text-xs text-slate-500 dark:text-zinc-400">Sin transacciones aún.</p>
                  )}
                    ...
                    return (
                      <div key={month} className="space-y-1 rounded-lg bg-slate-100/60 p-2 dark:bg-white/5">
                        <div className="flex items-center justify-between text-xs text-slate-500 dark:text-zinc-400">
                          <span>{month}</span>
                          <span>{formatMoney(data.ingresos - data.egresos, cuentaSeleccionada.moneda)}</span>
                        </div>
                        ...
                        <div className="flex justify-between text-[11px] text-slate-500 dark:text-zinc-400">
                          <span>+ {formatMoney(data.ingresos, cuentaSeleccionada.moneda)}</span>
                          <span>- {formatMoney(data.egresos, cuentaSeleccionada.moneda)}</span>
                        </div>
                      </div>
                    );
                  })}
              ...
              <div className="rounded-xl border border-slate-200/70 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-black/40">
                <p className="text-sm font-semibold text-slate-900 dark:text-white">Transacciones</p>
                ...
                <div className="mt-3 space-y-2 max-h-[380px] overflow-y-auto pr-1">
                  {filteredTxs.map((tx) => (
                    <TransactionListItem key={tx.id} tx={tx} onEdit={startTxEdit} onDelete={deleteTx} />
                  ))}
                  {filteredTxs.length === 0 && (
                    <p className="text-xs text-slate-500 dark:text-zinc-400">Aun no hay movimientos.</p>
                  )}
                </div>
              </div>
            </div>
        ...
        {!loadingSession && (
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="text-right">
                    ...
                    {cta.tasaApr && (
                      <p className="text-xs text-slate-500 dark:text-zinc-400">{cta.tasaApr}% APR</p>
                    )}
                  </div>
                </div>
                <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-600 dark:text-zinc-400">
                  {cta.limiteCredito ? (
                    <span>Límite: {formatMoney(Number(cta.limiteCredito), cta.moneda)}</span>
                  ) : null}
            ...
            {cuentas.length === 0 && (
              <p className="text-sm text-slate-500 dark:text-zinc-400">No tienes cuentas aún.</p>
            )}
          </div>
        )}
      </div>
            ...
            <div className="mb-4 flex items-center justify-between">
              <div>
                <p className="text-xs uppercase tracking-[0.3em] text-sky-400">
                  {editingTxId ? "Editar transacción" : "Registrar transacción"}
                </p>
              ...
              {editingId ? (
                <div className="rounded-xl border border-slate-200/70 bg-white/60 p-4 text-sm shadow-inner dark:border-white/10 dark:bg-white/5">
                  ...
                  <p className="mt-2 text-xs text-slate-500 dark:text-zinc-400">
                    Al guardar se generará una transacción interna con el ajuste indicado.
                  </p>
                </div>
              ) : (
                <div className="rounded-xl border border-dashed border-slate-300 px-4 py-3 text-sm text-slate-600 dark:border-white/20 dark:text-zinc-400">
                  El saldo se actualizará cuando registres transacciones o ajustes posteriores.
                </div>
              )}
